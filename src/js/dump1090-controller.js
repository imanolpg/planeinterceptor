...
setTimeout(main, 1000); // time for the process to be killed
...
function main() {
    ...
    setTimeout(reciver, 5000);
}


/**
 * Connects to the tcp streaming server and manages all data
 */
function reciver() {
    ...
    client.on('data', (chunk) => {
        ...
        plane = {
            ...
            flightID: (data[5] !== "") ? parseInt(data[5]) : data[5],
            ...
            track: (data[13] !== "") ? parseInt(data[13]) : data[13],
            latitude: (data[14] !== "") ? parseFloat(data[14]) : data[14],
            longitude: (data[15] !== "") ? parseFloat(data[15]) : data[15],
            verticalRate: (data[16] !== "") ? parseInt(data[16]) : data[16],
            squawk: (data[17] !== "") ? parseInt(data[17]) : data[17],
